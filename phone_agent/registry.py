...
orchestrator_agent = Agent(
    ...
    instructions=[
        ...
        # ============ 任务分解指导 ============
        "【步骤1-任务分解】对于复杂任务，先使用 `decompose_task` 工具获取详细步骤列表作为参考。",
        
        # ============ 混合执行策略 ============
        "【步骤2-智能执行】根据分解结果执行任务，采用混合策略：",
        "  - 可以合并：打开应用+搜索+基础筛选（如：打开大众点评，搜索日料，设置评分4.5以上、人均150以下）",
        "  - 必须独立：涉及'选择/挑选/决定'的步骤（如：从搜索结果中选择一家符合条件的餐厅）",
        "  - 必须独立：涉及'记录/保存'信息的步骤",
        "  - 可以合并：发送消息类操作",
        
        # ============ 筛选条件强调 ============
        "【关键】执行任何涉及筛选的步骤时，必须在任务描述中明确包含所有筛选条件：",
        "  - 错误示例：'选择一家餐厅'",
        "  - 正确示例：'从搜索结果中选择一家评分4.5以上且人均150元以下的餐厅'",
        "  - 如果找到的结果不符合条件（如人均超过150元），必须继续寻找或告知用户没有符合条件的选项。",
        
        # ============ 执行原则 ============
        "在执行下一个步骤之前，请务必根据上一个步骤的返回结果来调整计划。",
        "如果上一步失败了，尝试通过不同的方式重试或者询问用户。",
        "最终任务完成后，向用户汇报详细的结果，包括选择的具体信息（名称、价格、评分等）。"
    ],
    markdown=True,
    # 数据库存储 - 启用上下文记忆
    db=db,
